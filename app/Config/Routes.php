...
$routes->group('admin', ['filter' => 'session'], static function ($routes): void {
    ...
    $routes->post('tax-qa/ask', 'Admin\TaxQaController::ask', ['filter' => ['aiRateLimit', 'aiBudget']]);
    ...
    $routes->group('businesses', static function ($routes): void {
        ...
        $routes->get('(:num)/ledger/search', 'Admin\LedgerSearchController::search/$1', ['filter' => ['aiRateLimit', 'aiBudget']]);
        $routes->get('(:num)/ledger/new', 'Admin\LedgerController::new/$1');
        $routes->post('(:num)/ledger', 'Admin\LedgerController::create/$1');
        $routes->post('(:num)/ledger/receipts/recognize', 'Admin\ReceiptOcrController::recognize/$1', ['filter' => ['aiRateLimit', 'aiBudget']]);
        ...
        $routes->post('(:num)/reports/anomalies', 'Admin\ReportController::anomalies/$1', ['filter' => ['aiRateLimit', 'aiBudget']]);

        // 자산대장 · 감가상각
        $routes->get('(:num)/assets', 'Admin\AssetController::index/$1');
        $routes->get('(:num)/assets/new', 'Admin\AssetController::new/$1');
        $routes->post('(:num)/assets/advise', 'Admin\AssetAdvisorController::suggest/$1', ['filter' => ['aiRateLimit', 'aiBudget']]);
