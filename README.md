...
### 월간 예산 상한 (장기 누적 비용 보호)

레이트 리밋은 분 단위 폭주만 막을 뿐, 임계치 이하로 꾸준히 호출하는 **장기간 누적 비용**은 막지 못한다.
이를 위해 사용자 단위 **월간 호출 수/토큰 사용량**을 영속 카운터(DB)에 집계하고 상한 초과 시 차단한다(`AiBudgetFilter`).
레이트 리밋과 독립적으로 함께 적용되며(위 표와 동일한 7개 엔드포인트), 실제 외부 호출이 성공했을 때만
(`AnthropicClient` 호출 직후) 집계하므로 이력·규칙 폴백은 카운트되지 않는다.

- **정책**: 이번 달 누적 호출 수 또는 토큰 수(입력+출력)가 상한 이상이면 새 호출을 차단한다. 판정은 이번 호출 "이전"까지의 누적치 기준이라, 상한을 막 넘긴 호출 자체는 통과하고 다음 호출부터 막힌다(레이트 리밋과 동일한 보수적 원칙). 초과 시 POST(AJAX 어시스트)는 **HTTP 429**(에러 코드 `MONTHLY_CAP_EXCEEDED`, CSRF 토큰 동봉), GET(자연어 검색)은 플래시 에러와 함께 이전 페이지로 리다이렉트한다.
- **폴백 비차단**: `ANTHROPIC_API_KEY` 미설정 시 대상 엔드포인트는 외부 호출 없이 이력·규칙·검색 폴백만 하므로 상한을 적용하지 않는다.
- **동시성**: 카운터 증가는 원자적 UPDATE(산술 증가)로 처리해 동시 요청에도 유실 없이 정확히 누적된다.
- **설정(`.env`, 무배포 조정)**: `ai.monthlyCap.enabled`(기본 `true`) · `ai.monthlyCap.calls`(기본 `0`=무제한) · `ai.monthlyCap.tokens`(기본 `0`=무제한).
